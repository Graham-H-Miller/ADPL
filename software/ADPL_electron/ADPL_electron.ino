    ...
    if ((currentTime - last_publish_time) > PUBLISH_DELAY) {
        ...
        if(SDCARD){
            ...
            if(sDPublisher.publish(tempHXCI.temp, tempHXCO.temp, tempHTR.temp, tempHXHI.temp, tempHXHO.temp,
                ...
                Log.info("SD publish successful. Publish time: [%s",
                         Time.hour(), ":%s", Time.minute(), ":%s", Time.second(), "]");
                publishedSD = true;
            } else {
        ...
        if(publishedCell || publishedSD){
            Log.info("At least one publish was successful. Updating variables accordingly...");
